...
class TestEventsEndpoint(QueryTestCase):
    endpoint = EventsEndpoint

    @classmethod
    def setUpClass(cls) -> None:
        cls.events_default_kwargs = dict(
            client_params=ClientParams(limit=1, page_size=1, max_pages=1),
            asset_contract_address=cls.sample_contract,  # punk
            event_type=EventType.SUCCESSFUL,
        )

    def test_param_event_type_filters_properly(self):
        updated_kwargs = self.events_default_kwargs | dict(client_params=ClientParams(limit=5, page_size=5, max_pages=1))
    ...
    def test_param_auction_type_filters_properly(self):
        new_client_params = ClientParams(limit=5, page_size=5, max_pages=1)
    ...
    def test_param_only_opensea_true_filters_properly(self):
        new_client_params = ClientParams(limit=2, page_size=2, max_pages=1)
